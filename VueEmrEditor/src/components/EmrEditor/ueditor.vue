<template>
  <div class="ueditor-oak" >
    <!-- style="height:auto;" -->
    <div class="ueditor-toobar-oak" :hidden="!showToolbar">
      <!-- , '视图审阅' -->
      <Tab
            ...
            <div class="edui-default">
              <div class="edui-toolbar" style="display:flex">
                <div class="edui-box edui-button edui-for-orient">
                  <div :class="{'edui-state-hover':styleType_===false}" @click="layout(false)">
                ...
                <div class="edui-box edui-button edui-for-portrait">
                  <div
                    :class="{'edui-state-hover':styleType_===true}"
                    @click="layout(true)">
                    <div class="edui-button-wrap">
                      <div
                        unselectable="on"
      ...
      <div class="fixed-toobar" :hidden="true">
        <div class="edui-default">
    ...
    <div :hidden="!showToolbar">
      <NewTab :transMst="transMst"  v-on:updateOrderTreeData="updateOrderTreeData" />
    </div>

    <div
      id="myueditor" 
      class="ueditor-content-oak" 
      :style="{backgroundColor: selectedColor, height:showToolbar?'calc(100% - 90px)':'100%'}">
      <div class="ueditor-content" :style="getStyle()">
        <script
...
import Tab from '@/components/EmrEditor/common/tab.vue'
import innerHtml from '@/components/EmrEditor/innerHtml.js'
import VestigeReservation from '@/components/EmrEditor/vestigeReservation.vue'
import NewTab from '@/components/EmrEditor/common/filetab.vue'
...
export default {
  ...
  watch : {
    showToolbar:function(newVal, oldVal) {
        console.log("showToolbar --------", newVal)
    }
  },
  data() {
    return {
      editor: undefined,
      types: ['編輯模式', '只讀模式', '審閱模式', '留痕模式'],
      index: 0,
      actualChange: [],
      editHtml: undefined,
      fixedHtml: undefined,
  ...
  mounted: function() {
    ...
    // this.style.width = this.styleType ? '700px' : '1000px'
    // this.changeSize(this.styleType)
  ...
  methods: {
    ...
    getStyle() {
      if(this.showToolbar) {
        var index = this.sizes.findIndex((x) => x.value === this.selectedSize);
        // this.style.width = this.styleType ? '700px' : '1000px'
        var width = parseInt(this.sizes[index].width) + 5 + "mm";
        var height = parseInt(this.sizes[index].height) + 5 + "mm";
        this.style.width =  this.styleType_ ? width : height
    ...
    //   localStorage.setItem("SHEET_STYLETYPE", JSON.stringify(type))
    // },

    layout: function(type) {
        // this.style.width = type ? '700px' : '1000px'      
      this.styleType_ = type
      this.changeSize(this.selectedSize)
      
      const editor = this.editor
      if(editor.windows != undefined) {
        const head = editor.windows.document.getElementsByTagName('head')[0]
        const cssStyle = document.createElement('style')
        cssStyle.id = 'layout'
        cssStyle.innerText = '@page { size: landscape; }'
      ...
      localStorage.setItem("SHEET_STYLETYPE", JSON.stringify(type))
      console.log("SHEET_STYLETYPE", type)
    },
    changeSize(size) {
