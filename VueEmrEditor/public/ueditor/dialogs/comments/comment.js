...
UE.plugins['postil'] = function () {
    ...
    execCommand: function (cmdName, opt) {
      ...
      if (
        ...
        var markRange = range.applyInlineStyle(tag, {
          style:
            'background-color: rgb(255, 255, 0) !important;font-style: normal;',
          plugins: 'postil',
          content: opt.content,
          author: opt.author,
          date: opt.date,
  ...
  me.commands['unpostil'] = {
    ...
    execCommand: function () {
      ...
      if (
        range.collapsed &&
        !domUtils.findParentByTagName(range.startContainer, tag, true)
      ) {
        return
      }
    ...
    domUtils.findParent(
      ...
      function (node) {
        if (
          ...
          node.getAttribute('plugins') == 'postil'
        ) {
          ...
          var html = popup.formatHtml(
            '<div style="width:200px;">' +
              ...
              ': <p>' +
              content +
              '</p><div style="text-align:right;"><span onclick=$$._edittext() class="edui-clickable">编辑</span>&nbsp;<span onclick=$$._delete() class="edui-clickable">删除</span></div></div>'
          )
...
UE.registerUI('showpostil', function (editor, uiName) {
  ...
  var btn = new UE.ui.Button({
    ...
    cssRules:
      ...
      'dialogs/comments/showPostil.png) no-repeat 3px 2px / 74% !important',
    onclick: function () {
...
UE.registerUI('hidepostil', function (editor, uiName) {
  ...
  var btn = new UE.ui.Button({
    ...
    cssRules:
      'background: url(' +
      editor.options.UEDITOR_HOME_URL +
      'dialogs/comments/hidePostil.png) no-repeat 3px 2px / 76% !important',
    onclick: function () {
      var markList = me.document.getElementsByTagName('mark')
      UE.utils.each(markList, function (value, key) {
