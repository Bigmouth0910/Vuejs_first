/**
 * Created by peng.hongbao on 2019/7/21.
 * desc EMR电子病历编辑器，事件绑定 ，实现痕迹保留
 * 痕迹保留实现思路
  ...
  checkboxChange: function (target, value, key) {
    ...
    target.addEventListener('click', function (e) {
      ...
      if (e.target.tagName == 'INPUT') {
        ...
        var newModel = model
        var indexm = 0
        for (var md = 0; md < model.bindData.length; md++) {
          if (newValue.length > 0) {
            for (var nv = 0; nv < newValue.length; nv++) {
              if (model.bindData[md].VALUE === newValue[nv]) {
                newModel.bindData[md].SELECTED = true
              }
            }
          } else {
            newModel.bindData[md].SELECTED = false
          }
        }
        for (var tc = 0; tc < target.children.length; tc++) {
          if (target.children[tc].tagName == 'LABEL') {
            for (var tcc = 0; tcc < target.children[tc].children.length; tcc++) {
              if (newValue.length > 0) {
                for (var nv = 0; nv < newValue.length; nv++) {
                  if (target.children[tc].children[tcc].value === newValue[nv]) {
                    target.children[tc].children[tcc].setAttribute('checked', true)
                  } else {
                    target.children[tc].children[tcc].removeAttribute('checked')
                  }
                }
              } else {
                target.children[tc].children[tcc].removeAttribute('checked')
        ...
        target.removeAttribute('obj')
        target.setAttribute('obj', JSON.stringify(newModel))
        if (newValue != oldValue) {
          var time = new Date().format('yyyy-MM-dd HH:mm:ss')
          that.changes[key] = {
            date: time,
